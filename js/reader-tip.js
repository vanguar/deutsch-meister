/* ═══════════════════════════════════════════════
   ...
   Перевод и разбор употребления — разные поля глоссария: ru (короткий
   перевод, он же на карточке) и note (обороты, отделяемые приставки,
   пояснения). В тултипе note идёт отдельной строкой под переводом,
   приглушённым цветом.
...
const ReaderTip = (() => {
  ...
  function render(word, info, opts) {
    ...
    // note — разбор употребления (обороты, отделяемые приставки). Отдельной
    // строкой приглушённым цветом, а не в одну строку с переводом: на
    // карточке note не показывается вовсе, и смешивать их нельзя.
    if (e.note) parts.push(`<span class="word-tip-note">${esc(e.note)}</span>`);
