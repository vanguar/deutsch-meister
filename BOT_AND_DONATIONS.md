...
11. **Пересоздание токена в BotFather УДАЛЯЕТ вебхук.** Одна причина — два
    симптома, и по отдельности они сбивают с толку: Telegram перестаёт слать
    апдейты (вебхук снесён), а функция не может ответить (в окружении лежит
    старый токен). Снаружи всё выглядит живым: health отдаёт 200, POST с
    секретом отдаёт 200, в логах Vercel тишина — потому что Telegram туда
    просто не стучится. **Проверять обе вещи разом:**
    ```bash
    scripts/bot_doctor.sh <ТОКЕН>          # диагноз
    scripts/bot_doctor.sh <ТОКЕН> --fix    # новый секрет + деплой + setWebhook
    ```
    Или прямо в браузере, без токена:
    `https://deutsch-meister-puce.vercel.app/api/webhook?probe=1` —
    поле `verdict` называет причину словами. Секретов в ответе нет:
    `getWebhookInfo` никогда не возвращает `secret_token`.

12. **`WEBHOOK_SECRET` и `BOT_TOKEN` помечены Sensitive.** Прочитать их
    нельзя ничем — ни в панели, ни через `vercel env pull` (приезжают
    пустыми независимо от реального значения). Поэтому при починке вебхука
    секрет не «вспоминают», а генерируют новый и прописывают с обеих
    сторон. И поэтому же пустое значение в `vercel env pull` — НЕ повод
    считать переменную сломанной.

13. **Правка CSS/JS сайта деплоя на Vercel не требует.** Фронтенд живёт на
    GitHub Pages; на Vercel — только бот (`api/webhook.py` отвечает на любой
    путь домена). Лишние `vercel --prod` ничего не чинят и только
    перекладывают прод бота.
