...
но код возврата не меняют):
  ...
  * необязательное поле "rv" (рифмованный перевод строки), если есть, —
    непустая строка;
            ...
            for si, sent in enumerate(sentences):
                ...
                if not ru.strip():
                    err('%s: [%d.%d] нет перевода (ru): %s'
                        % (label, pi, si, de[:60]))
                # rv — рифмованный перевод стихов. Поле необязательное:
                # книга может быть без рифм вовсе, а рифмованная — набиваться
                # по главам. Но пустая строка в rv хуже отсутствия поля:
                # ридер покажет пустую строку вместо строфы.
                if 'rv' in (sent or {}):
                    rv = sent.get('rv')
                    if not isinstance(rv, str) or not rv.strip():
                        err('%s: [%d.%d] rv пустое или не строка: %s'
                            % (label, pi, si, de[:60]))
